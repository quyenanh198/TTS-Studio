...
Đã bỏ tím `#7c5cff` (AI-slop) và gradient logo. Contrast đo thực tế trong app: xem `design-tokens.json → contrast`.
...
- Thang: H1 22/bold, card title 15/bold, body 13–14, meta 11–12, tag 10.5 uppercase. Không nhỏ hơn 10.5px.
...
| Field | `.input`, `<Field label help>` | hover border, focus ring 2px, disabled, placeholder subtle |
| Segmented | `<Segmented>` (`aria-pressed`) | thay thế nhóm nút tab tự chế |
...
- Nút chỉ-icon luôn có `aria-label`; danh sách giọng là `listbox/option`, điều hướng bằng Enter/Space; nav dùng `aria-current`.
